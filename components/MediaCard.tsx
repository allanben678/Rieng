...
const cardClasses = "group relative bg-[#080808] border border-white/5 rounded-[2rem] overflow-hidden hover:border-primary/50 hover:shadow-[0_0_30px_rgba(255,107,53,0.15)] transition-all duration-300 flex flex-col h-[340px] cursor-pointer w-full text-left";
...
export default function MediaCard({
    ...
    <>
      <div className="relative h-[220px] w-full overflow-hidden flex-shrink-0">
        ...
        {videoId && (
          <div className="absolute inset-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="w-16 h-16 rounded-full bg-primary/90 backdrop-blur-sm flex items-center justify-center shadow-[0_0_30px_rgba(255,107,53,0.6)] border border-white/20">
      ...
      <div className="p-6 md:p-8 pt-0 flex flex-col flex-1 transform transition-transform duration-300 group-hover:-translate-y-2 relative z-20 -mt-8">
        ...
        <div className="mt-auto flex items-center text-primary font-black text-xs tracking-widest uppercase transition-transform group-hover:translate-x-2">
          {videoId ? 'Watch Video' : category.toLowerCase() === 'music' ? 'Listen' : 'Read Story'} <span className="ml-2 text-lg">→</span>
        </div>
      </div>
    </>
  );
  ...
  if (videoId && onVideoClick) {
    return (
      <button
        onClick={() => onVideoClick(videoId, title)}
